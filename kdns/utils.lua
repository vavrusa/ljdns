...
-- FFI + C code
local cutil = ffi.C, ffi.load(package.searchpath('kdns_clib', package.cpath))
ffi.cdef[[
...
local tmp1, tmp2 = ffi.new('uint8_t[128]'), ffi.new('uint8_t[128]')
local function dnamecmp2(lhs, rhs)
	local l1, l2, i1, i2 = 0, 0, 128, 128
	local lb, rb = lhs.bytes, rhs.bytes
	while lb[l1] > 0 do
			i1 = i1 - 1
			tmp1[i1] = l1
			l1 = l1 + lb[l1] + 1
	...
	local ret = i2 - i1
	local ltop, rtop = tmp1 + i1, tmp2 + i2
	for i = 0, math.min(128 - i1, 128 - i2) - 1 do
		local lb1, lb2 = lb + ltop[i], rb + rtop[i]
		local lbret = C.memcmp(lb1 + 1, lb2 + 1, math.min(lb1[0], lb2[0]))
		if lbret == 0 then lbret = lb1[0] - lb2[0] end
		if lbret ~= 0 then ret = lbret break end
	end
	return ret
end
